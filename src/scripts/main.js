...
const checkLoose = () => {
  ...
  for (let i = 0; i < 4; i++) {
    const colCells = [fieldRows[0].children[i], fieldRows[1].children[i], fieldRows[2].children[i], fieldRows[3].children[i]];
...
const moveRight = (row) => {
  ...
  for (let i = values.length - 1; i > 0; i--) {
    if (values[i] === (values[i - 1] || values[i - 2] || values[i - 3]) && values[i] !== 0) {
      countScore(values[i]);
      values[i] *= 2;
      values[i - 1] = 0;
    }
...
const moveFn = (e) => {
  switch (e) {
    case 'ArrowRight':
      buttonStart.hasAttribute('start')
      && addCellNumber();
